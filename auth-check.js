...
async function checkAdminRole() {
    try {
        ...
        const adminEmails = ['davidheh15@gmail.com', 'timeforbeauty67n@gmail.com'];
...
async function checkResetPasswordAccess() {
    try {
        ...
        if (error) {
            console.error('Greška pri proveri sesije:', error);
            return; // Dozvoli pristup reset password stranici
        }

        // Ako je korisnik već prijavljen, preusmeri na odgovarajuću stranicu
        if (session && session.user) {
            const isAdmin = await checkAdminRole();
            if (isAdmin) {
        ...
        // Dozvoli pristup reset password stranici
    }
}
